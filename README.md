...
Требуется Python 3.12+ (кроме 3.14.1 — ограничение NetworkX 3.7);
GPU, облако и интернет во время расчёта не нужны. Проверено на Python 3.14.6.
...
### Свои файлы: локальный мастер импорта

После установки тех же зависимостей запустите:

```powershell
powershell -ExecutionPolicy Bypass -File .\run_app.ps1
```

Откройте `http://127.0.0.1:8765`. Мастер принимает CSV, TSV, XLSX, JSON, JSONL
и Parquet: один файл переводов либо отдельные таблицы, сопоставление колонок,
проверку, анализ и историю. Пользовательский профиль допускает другой объём,
валюту и период; неизвестные seed/depth не выдумываются. Профиль HackAlem
сохраняет все строгие проверки и прежние схемы CSV. Данные мастера хранятся
отдельно в исключённом из Git `workbench_data/`.

[Руководство мастера](docs/workbench.md) — примеры данных, деньги и точность,
ограничения, безопасность и очистка. Это приложение для одного локального
пользователя, не для публичного хостинга. Разделы ниже описывают исходный
строгий пайплайн; отличия пользовательского режима приведены в руководстве.

Для первого запуска есть [маленький синтетический пример](examples/workbench/README.md).
Проверенные возможности и границы проверки — в [протоколе мастера](docs/workbench-verification.md).
...
не требуются для обычного запуска аналитического пайплайна.
