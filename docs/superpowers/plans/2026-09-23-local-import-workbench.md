...
- [ ] Написать тест одинаковой сети во всех форматах, сохранив ID `001` и `A-2`.
- [ ] Зафиксировать отказы: дробный/пустой ID, недостаточная точность суммы,
  формулы XLSX, смешанные валюты, пустой граф, неизвестный конец ребра, лимиты.
- [ ] Реализовать чтение ограниченных таблиц и строгую Decimal-нормализацию.
- [ ] Проверить: `.venv\Scripts\python.exe -m unittest discover -s tests -p test_importing.py -v`.
- [ ] Закоммитить модуль и тесты после интеграционного ревью.
...
- [ ] Написать тест generic из одного файла и из трёх согласованных таблиц.
- [ ] Зафиксировать отсутствие terminal/transit при неизвестной полноте.
- [ ] Повторно использовать сборку метрик и компонентную кластеризацию через
  внутреннее стабильное отображение строковых ID; не менять исходные ID экспорта.
- [ ] Сохранить report.json, result.json, три CSV и graph_view.html; report содержит
  версии правил, ограничения, параметры и хеши файлов.
- [ ] Проверить: `.venv\Scripts\python.exe -m unittest discover -s tests -p test_workbench_analysis.py -v`.
...
- [ ] Написать HTTP-тесты Host/Origin/token/traversal/размеров и сохранения истории.
- [ ] Реализовать случайные upload/run ID, атомарные состояния и worker с одной задачей.
- [ ] Незавершённые запуски после рестарта отмечать interrupted; ошибки не публикуют артефакты.
- [ ] Проверить: `.venv\Scripts\python.exe -m unittest discover -s tests -p test_workbench_server.py -v`.
...
- [ ] Реализовать четыре шага с настоящими label, прогрессом и live-ошибками.
- [ ] Предлагать mapping по известным именам колонок; изменения отменяют preview.
- [ ] Через authenticated fetch открывать результат в sandboxed iframe Blob;
  скачивание CSV также через fetch, без токенов в URL.
- [ ] Отображать строковые ID, валюту и неизвестные seed/depth корректно;
  старый offline payload сохраняет поведение по умолчанию.
- [ ] Playwright проходит импорт CSV, исправление mapping, запуск, граф, CSV,
  историю после reload, проверку узкой ширины и ошибки сервера.

```javascript
assert.equal(await page.locator('#run-status').textContent(), 'Готово');
assert.equal(await page.locator('#run-history [data-run-id]').count(), 1);
...
- [ ] Зафиксировать зависимости и добавить workbench_data/ в gitignore.
- [ ] Запустить весь unittest, старые 17 UI-сценариев и новый мастер.
- [ ] Повторить HackAlem и сравнить три CSV побайтово с текущими результатами.
- [ ] Проверить отображение ошибок, локальную приватность и весь контракт спецификации.
- [ ] Записать фактические результаты, ограничения и команду `powershell -ExecutionPolicy Bypass -File .\run_app.ps1`.
- [ ] Закоммитить проверенный результат; не публиковать реальные загрузки и не выполнять push без отдельной просьбы.
